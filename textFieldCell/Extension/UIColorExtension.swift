...
let tmpView = UIView.init(frame: CGRect(self.view.frame.size.width/CGFloat(colors.count)*CGFloat(i), 100, self.view.frame.size.width/CGFloat(colors.count), 100))
i++
...
extension UIColor {
    ...
    class func colorWithHexString(string:String, alpha:Float = 1.0) -> UIColor {
        let colorString = string.replacingOccurrences(of:"#", with: "").uppercased()
    ...
    class func colorHexComponent(string:String, start:Int, length:Int) -> Float {
        let subString = string.sub(start: start, length: length)
...
//    }
    ...
    class func colorWithRGB(rgb:String) -> UIColor {
        ...
        rgbValue = rgbValue.trimmingCharacters(in: NSCharacterSet(charactersIn:"rgba( )") as CharacterSet)
    ...
    class func colorWithCSS(css:String) -> UIColor {
        if css.hasPrefix("rgb") {
            return colorWithRGB(rgb: css)
        } else if css.hasPrefix("hsl") {
            return colorWithHSL(hsl: css)
        } else if css.hasPrefix("0x") {
            var hexInt:UInt32 = UInt32()
            Scanner(string: css).scanHexInt32(&hexInt)
            return colorWithHex(hex: hexInt)
        } else {
            return colorWithHexString(string: css)
        }
    }
    ...
    class func colorWithHSL(hsl:String) -> UIColor {
        ...
        hslValue = hslValue.trimmingCharacters(in: NSCharacterSet(charactersIn:"hsla(%) ") as CharacterSet)
